...
import { recordSnapshot, setPromotionPending } from "../Helper/history.js";
...
function GlobalEvent() {
  ...
  let draggedGhostEl = null;
  let pointerId = null;
  let dragSourceSquareEl = null;
  let pendingDrag = null; // { pointerId, imgEl, fromId, startX, startY }
  const DRAG_THRESHOLD_PX = 6;

  function getSquareIdFromTarget(target) {
    if (!target) return null;
    if (target.localName === "span") return target.parentNode?.id ?? null;
    if (target.localName === "img") return target.parentNode?.id ?? null;
    if (target.id) return target.id;
    return null;
  }

  function startMoveForSquare(square) {
    if (!square?.piece) return false;
    const pieceName = square.piece.piece_name;

    // If it's not your turn, don't start a move
    if (
      (pieceName.includes("WHITE") && inTurn === "black") ||
      (pieceName.includes("BLACK") && inTurn === "white")
    ) {
      return false;
    }
  ...
  function cleanupCustomDrag({ clear = true } = {}) {
    if (draggedGhostEl?.parentNode) draggedGhostEl.parentNode.removeChild(draggedGhostEl);
    draggedGhostEl = null;

    if (dragSourceSquareEl) dragSourceSquareEl.classList.remove("draggingSource");
    dragSourceSquareEl = null;
    draggedImgEl = null;
  ...
  function moveGhostToPointer(ev) {
    if (!draggedGhostEl) return;
    const rect = draggedGhostEl.getBoundingClientRect();
    draggedGhostEl.style.left = `${ev.clientX - rect.width / 2}px`;
    draggedGhostEl.style.top = `${ev.clientY - rect.height / 2}px`;
  }

  // Custom drag (pointer-based) so the piece feels “real” and not ghosty
  ROOT_DIV.addEventListener("pointerdown", function (event) {
    if (event.target?.localName !== "img") return;
    if (pointerId !== null) return;

    const fromId = getSquareIdFromTarget(event.target);
    if (!fromId) return;

    // Don't start dragging immediately — allow normal click-to-move.
    // We'll start a drag only after the pointer moves a few pixels.
    pendingDrag = {
  ...
  ROOT_DIV.addEventListener("pointermove", function (event) {
    if (dragStarted) {
      if (event.pointerId !== pointerId) return;
      moveGhostToPointer(event);
      return;
    }
    ...
    const square = keySquareMapper[pendingDrag.fromId];
    if (!square?.piece) {
      pendingDrag = null;
      return;
    }

    const ok = startMoveForSquare(square);
    if (!ok) {
      pendingDrag = null;
      return;
    }
    ...
    draggedImgEl = pendingDrag.imgEl;
    dragSourceSquareEl = draggedImgEl?.parentNode ?? null;
    if (dragSourceSquareEl) dragSourceSquareEl.classList.add("draggingSource");

    // Create a visual “dragging” piece that follows the cursor
    draggedGhostEl = draggedImgEl.cloneNode(true);
    draggedGhostEl.classList.add("dragGhost");
    draggedGhostEl.draggable = false;
    document.body.appendChild(draggedGhostEl);

    moveGhostToPointer(event);
    try {
      ROOT_DIV.setPointerCapture(pointerId);
    } catch (e) {
      // ignore
    }
  ...
  ROOT_DIV.addEventListener("pointerup", function (event) {
    if (!dragStarted) {
      // This was a normal click (or tiny movement) — let the click handler run.
    ...
    const dropId = getSquareIdFromTarget(el);

    if (dropId) {
      const dropSquare = keySquareMapper[dropId];
      if (dropSquare?.highlight || dropSquare?.captureHighlight) {
        clearPreviousSelfHighlight(selfHighlightState);
        moveElement(moveState, dropId);
        cleanupCustomDrag({ clear: false });
        return;
      }
    }

    cleanupCustomDrag({ clear: true });
  });

  ROOT_DIV.addEventListener("pointercancel", function (event) {
    if (!dragStarted) {
      if (pendingDrag && event.pointerId === pendingDrag.pointerId) {
        pendingDrag = null;
      }
...
export { GlobalEvent, movePieceFromXToY, getInTurn, setInTurn, resetInteractionState };
